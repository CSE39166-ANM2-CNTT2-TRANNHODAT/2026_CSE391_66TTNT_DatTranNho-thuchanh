...
- Em thấy dòng thông báo"Hello from JavaScript!"
  -Điều này cho em biết:
  +java scrip đọc và thực thi "main.js"
  +java scrip thực hiện lệnh:"console.log()"
  +java scrip kiểm tra môi trường hoạt động
...
> a) JavaScript chạy ở đâu? Cả hai(cả ở trình duyệt và server)
> b) HTML, CSS, JavaScript mỗi phần chịu trách nhiệm chính về điều gì?
>
> - HTML: Xây dựng cấu trúc và nội dung thô của trang web
> - CSS: Chịu trách nhiệm về hình thức, định dạng kiểu dáng, màu sắc và bố cục
> - JavaScript: Tạo ra các hành động và tính tương tác
...
- ## Dòng log hiển thị"Xin chào,mình là Dat, năm nay mình 20 tuổi."
  - thiếu dấu";" thì code vẫn chạy
  - thiếu dấu"+" thì code không chạy nữa
...
- console báo lỗi và chỉ dòng lỗi
- đọc lỗi và sửa lỗi.Nếu sau 1 thời gian mà không tìm được lỗi thì em dùng AL để sửa code rồi so sánh với đoạn code của mình.và rút kinh nghiệm từ lỗi đó.
-
...
- khi score >= 8 thì chương trình in "giỏi"
- khi score >= 6.5 thì chương trình in "khá"
- khi score >= 5 thì chương trình in "trung bình"
- các trường hợp còn lại chương trình sẽ in "yếu"
- cấu trúc if/else là cấu trúc rẽ nhánh như ngã 3 đường.Giả sử,ta đang ở trường.Ta sẽ dựa vào đường đi từ trường về nhà thì khi đến ngã 3,ta phải chọn 1 trong 3 ngã rẻ để về nhà
...
>   - Tên hàm: Là tên gọi dùng để đại diện cho hàm, giúp bạn có thể gọi hàm đó ở bất cứ đâu trong chương trình.......
>   - Tham số (parameters): Là các giá trị đầu vào được đặt trong dấu ngoặc đơn () ngay sau tên hàm
>   - Thân hàm (body): Là tập hợp các câu lệnh thực hiện một công việc cụ thể, được bao bọc bởi cặp ngoặc nhọn {}....
>   - Giá trị trả về (return): Là kết quả cuối cùng mà hàm đưa ra sau khi thực hiện xong các câu lệnh, thường được xác định bởi từ khóa return
> - Ưu điểm của việc dùng hàm thay vì lặp lại cùng một đoạn code nhiều lần là gì?

- dễ bảo trì và sửa lỗi
- code rõ ràng,ngắn gọn
- có thể tái sử dụng
...
- phần khó nhất : là logic rẻ nhánh trong if/else
  -em dùng AL phân tích 1 bài toán cụ thể và tìm hiểu từ gốc rễ của bài toán.Sau đó,hình dung ra các nhánh rẽ của bài toán.
...
- `document.getElementById("status")`:dùng để truy xuất một phần tử cụ thể trong trang HTML thông qua thuộc tính id của nó
  > - Sự kiện `"click"` xảy ra hi người dùng nhấn chuột và thả ra trên một phần tử nhất định
  > - Trong đoạn code trên, khi nhấn nút `btnHello`, điều thay đổi trên trang:
        + Nội dung trang văn bản
        + giao diện
...
- sự kiện "input":Thay đổi dữ liệu
- sự kiện "click": tương tác vật lý
...
> - DOM: mô hình các đối tượng tài liệu dùng để truy xuất và thao tác trên file html
> - vd dòng lệnh:
...
1. Khi vừa tải trang (chưa tương tác).
   [Ảnh ở đây]("D:\D2026_CSE391_66ANM2_DatTranNho-thuchanh\th3-dom(khi chưa thao tác).png")
2. Sau khi nhấn “Chào”.
   [Ảnh ở đây]("D:\D2026_CSE391_66ANM2_DatTranNho-thuchanh\th3-dom(khi nhấn 'chào').png")
3. Sau khi đổi nền sang màu đỏ.
   [Ảnh ở đây]("D:\D2026_CSE391_66ANM2_DatTranNho-thuchanh\th3-dom(khi đổi màu đỏ).png")
4. Khi gõ tên và nhìn thấy lời chào xuất hiện.
   [Ảnh ở đây]("D:\D2026_CSE391_66ANM2_DatTranNho-thuchanh\th3-dom(khi nhập tên).png")
...
- trả lời:
  +giống:Đều dùng để lắng nghe sự kiện click trên phần tử có ID btnHello và hiển thị thông báo alert
  +khác:
  ~S thuần dùng document.getElementById("id") :dài và tường minh
  ~jQuery dùng $("#id"): ngắn và giống CSS selector

> - Điểm khác nhau về cú pháp là gì (`document.getElementById` vs `$("#id")`, `addEventListener` vs `.on`)?
> - Điểm khác biệt:

    + Chọn phần tử:
    ~ (document.getElementById): Chỉ dùng để tìm theo ID. Cú pháp dài và trả về trực tiếp phần tử HTML
    ~ ($("#id")): Dùng chung một cú pháp cho cả ID, Class, Attribute

> .Em hãy tra cứu nhanh “What is jQuery used for?” và ghi 2 ý chính:

1. Giúp viết JavaScript đơn giản và ngắn hơn khi thao tác với các thành phần trên web
2. Dễ dàng tạo hiệu ứng, xử lý sự kiện và truyền tải dữ liệu giữa trình duyệt và máy chủ.
...
>    - Biến \& kiểu dữ liệu: [ ] Chưa hiểu [ ] Tạm ổn [0] Khá rõ
>    - If/else \& hàm: [ ] Chưa hiểu [0] Tạm ổn [ ] Khá rõ
>    - DOM \& sự kiện: [ ] Chưa hiểu [0] Tạm ổn [ ] Khá rõ
